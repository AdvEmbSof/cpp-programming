...
void play_with_classes() {
...

  {
    ...
    // The following construction is forbidden because the constructor is now declared explicit
    // well_defined::String s4 = 10;
    // LOG_DBG("s4 is %s (length %d)", s4.getContent(), s4.getSize());

    // The following construction is forbidden because the constructor is now declared explicit
    // well_defined::String s5 = '1234';
  }
